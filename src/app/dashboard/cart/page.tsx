...
export default function CartPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 sm:mb-6">장바구니</h1>
      ...
      {cart.length === 0 ? (
        ...
        <>
          <div className="divide-y divide-gray-100">
            {cartProducts.map(cp => (
              <div key={cp.productId} className="flex flex-col sm:flex-row gap-3 sm:gap-4 py-4 sm:py-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm font-medium text-gray-900 mb-1 truncate">{cp.product.name}</p>
                      <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">{cp.unitPrice.toLocaleString()}원</p>
                    </div>
                    <p className="text-xs sm:text-sm font-bold text-gray-900 shrink-0 sm:hidden">{cp.total.toLocaleString()}원</p>
                  </div>
                ...
                <p className="text-sm font-bold text-gray-900 shrink-0 pt-1 hidden sm:block">{cp.total.toLocaleString()}원</p>
              </div>
            ))}
          </div>

          <div className="mt-5 sm:mt-6 pt-5 sm:pt-6 border-t border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="text-xs text-gray-400">{grade} 등급 적용</p>
              <p className="text-xl sm:text-2xl font-bold text-gray-900 mt-1">{totalAmount.toLocaleString()}원</p>
            </div>
            <button onClick={handleOrder} disabled={ordering}
              className="w-full sm:w-auto bg-gray-900 text-white px-8 py-3 rounded-lg text-sm font-medium hover:bg-black transition-colors disabled:opacity-50">
              {ordering ? '주문 중...' : '주문하기'}
            </button>
          </div>
        </>
